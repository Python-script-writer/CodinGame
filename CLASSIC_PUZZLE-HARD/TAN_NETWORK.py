...
class Heap(object):
    ...
    def push(self, priority, item):assert priority >= 0;heapq.heappush(self._heap, (priority, item))
    def pop(self):item = heapq.heappop(self._heap)[1];return item
...
for i in range(m):
    ...
    if s1 not in graph:
        graph[s1] = []
    graph[s1].append([s2, distance(s1,s2)])
...
while u != e :
    ...
    if len(minheap)==0:pos=0;break
    else:u = minheap.pop()


if pos==1:
    revstops = []
    while u!='NULL':revstops.append(stops[u][0]);u = parent[u]
    revstops = revstops[::-1]
    for i in revstops:print(i)
